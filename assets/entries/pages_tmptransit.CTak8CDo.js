...
<link href="https://fonts.googleapis.com/css2?family=Archivo:wdth,wght@62..125,100..900&amp;family=Newsreader:ital,opsz,wght@0,6..72,300..700;1,6..72,300..700&amp;family=IBM+Plex+Mono:wght@400;500;600&amp;display=swap" rel="stylesheet">
<style>
:root{
  --paper:#FBFBF9;
  --ink:#15181F;
...
function drawBoard(){
  boardRows.innerHTML='';
  [...CITIES].sort((a,b)=>b.overall-a.overall).forEach((c,i)=>{
    const row=document.createElement('div');
    row.className='brow'+(selected.includes(c.name)?' sel':'');
    row.style.animationDelay=(i*70)+'ms';
    const bm=bestMetric(c);
    const st=c.overall>=80?['GOOD SERVICE','ok']:c.overall>=70?['MINOR DELAYS','mid']:['DISRUPTED','bad'];
    row.innerHTML='<span class="r">'+String(i+1).padStart(2,'0')+'</span>'
      +'<span class="dot" style="background:'+c.c+'"></span>'
      +'<span class="dest">'+c.name+'</span>'
      +'<span class="pl">BEST: '+bm.label.toUpperCase()+'</span>'
      +'<span class="sc">'+c.overall.toFixed(1)+'</span>'
      +'<span class="st '+st[1]+'">'+st[0]+'</span>';
    row.addEventListener('click',()=>toggleCity(c.name));
...
function tickClock(){
  ...
  boardClock.innerHTML=p(d.getHours())+'<span class="blink">:</span>'+p(d.getMinutes())+'<span class="blink">:</span>'+p(d.getSeconds());
}
setInterval(tickClock,1000);

/* ================= Argument map ================= */
const argsvg=document.getElementById('argmap');
const AM_LINES=[
  {name:'Index Line — the spine',c:'#15181F',w:9,target:'s-origin',pts:[[80,280],[920,280]]},
  {name:'Legibility Line — Finding 01',c:'#F2C500',w:6,target:'s-f1',pts:[[260,280],[300,280],[420,160],[800,160],[920,280]]},
  {name:'Frequency Line — Finding 02',c:'#00A84D',w:6,target:'s-f2',pts:[[260,280],[320,280],[380,220],[840,220],[900,280],[920,280]]},
  {name:'Transfer Line — Finding 03',c:'#0091D2',w:6,target:'s-f3',pts:[[260,280],[320,280],[380,340],[840,340],[900,280],[920,280]]},
  {name:'Equity Line — Section 6',c:'#6E51C8',w:6,target:'s-equity',pts:[[260,280],[300,280],[420,400],[800,400],[920,280]]},
];
const AM_STATIONS=[
  {x:80,y:280,c:'#DC241F',big:1,lbl:'ORIGIN',sub:'why usability',ly:312,target:'s-origin'},
  {x:260,y:280,c:'#F39700',big:1,lbl:'THE INDEX',sub:'six measures',ly:252,target:'s-index'},
  {x:920,y:280,c:'#15181F',big:1,lbl:'TERMINUS',sub:'what to do monday',ly:312,target:'s-terminus'},
